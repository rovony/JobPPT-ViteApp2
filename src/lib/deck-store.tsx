...
export function useKeyboardNav({ onToggleFullscreen, shareViewer = false, isFullscreen = false } = {}) {
  const { next, prev, toggleMode, togglePresenter, presenter, setPresenter, mode } = useDeck();
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === 'ArrowRight' || e.key === ' ' || e.key === 'PageDown') { e.preventDefault(); next(); }
        ...
        if (shareViewer) {
          // Share help text: close overview, or exit fullscreen only — not "enter fullscreen" from slide.
          if (mode === 'overview') { e.preventDefault(); toggleMode(); return; }
          if (isFullscreen) { e.preventDefault(); onToggleFullscreen?.(); return; }
          return;
        }
        e.preventDefault();
      ...
      else if (e.key.toLowerCase() === 'p') { if (shareViewer) return; e.preventDefault(); togglePresenter(); }
      else if (e.key.toLowerCase() === 'f') { e.preventDefault(); onToggleFullscreen?.(); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [next, prev, toggleMode, togglePresenter, presenter, setPresenter, onToggleFullscreen, shareViewer, isFullscreen, mode]);
}
