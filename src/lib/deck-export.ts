...
// Default wait after each slide nav before rasterising. Per-slide and
// per-deck `defaultSettleMs` override this. **Lower = faster export.**
// Heavy animation / GSAP slides should set `export.slideSettleMs[id]` in
// the deck manifest (see qp2-seminar manifest). Previously 2400; 1600
// matches typical framer card transitions; opt up for long timelines.
const FALLBACK_DEFAULT_SETTLE_MS = 1600;
  ...
  try {
    ...
    for (let i = 0; i < totalSlides; i += 1) {
      ...
      try {
        ...
        // Let the main thread process toast/UI updates before the next slide.
