  ...
  return (
    <SlideGrid dataCase="amber" areas={STANDARD_AREAS}>
      <Eyebrow color="var(--amber)" delay={T.eyebrow}>
        Framework · Five recurring judgments
      </Eyebrow>

      {/* Option A · recurring-judgments framing.
          Picks up S3's "one through-line question" thread and uses the
          diagram's own grammar (5 inputs → 1 judgment → 3 outcomes).
          Headline reads as a contract for the next 35 minutes:
          three different drugs, three different questions, ONE
          recurring set of five judgments. */}
      <Headline delay={T.headline} maxChars={48}>
        Every case is a different drug and a different question —
        <br />
        but the same{' '}
      ...
      {/* size="lead" — the default --fs-slide-subhead token (~13-18px)
          ...
          with the headline. */}
      <Subhead delay={T.subhead} maxChars={72} size="lead">
        When the model becomes the evidence, these are the calls that have to land.
      </Subhead>
      ...
      <Footer
        delay={reduced ? 0 : 6.6}
        kicker="Five judgments · one decision · three case outcomes"
      />
    </SlideGrid>
  );
}
