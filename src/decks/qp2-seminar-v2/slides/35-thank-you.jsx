 ...
 *
 * Direction adopted: THEMES CONSTELLATION RE-BLOOM (Direction 1).
 * The constellation that has carried the deck's structural through-
 * line on slides 30, 31, and 33 assembles one final time as the
 * full-canvas illustration — five nodes settle, ten edges trace,
 * the five QP2 theme glyphs (⇌ ◎ ⊕ ◇ ◈ from themes.js) bloom
 * onto the nodes, the central convergence pulses amber, and "Thank
 * you." resolves on top.
 *
 * Why this and not the other two directions: the constellation has
 * become the deck's signature. Re-blooming it earns "wow" without
 * inventing a new visual language at the very last moment — the
 * audience reads "all five themes converge" as the closing argument
 * rather than as decoration.
 *
 * Motion arc (seconds; full sequence ≈ 5.0s, then ambient hold):
 *   0.10  chrome lands (eyebrow + page meta)
 *   0.20  five nodes begin settling (60ms stagger, overshoot ease)
 *   0.55  K5 edges trace L→R (45ms stagger), each flashing amber
 *         then settling to cream
 *   1.30  central convergence pulses amber
 *   1.45  "Thank" lands
 *   1.85  italic amber "you." sweeps in from the right
 *   2.10  theme glyphs bloom onto the five nodes (80ms stagger)
 *   2.20  calligraphic underline draws L→R via pathLength
 *   2.50  "Questions welcome." subhead fades up
 *   2.80  author block resolves
 *   2.95  contact links arrive
 *   3.20  theme titles fade in around the perimeter
 *   ≈4.5  perimeter wave begins (light circumnavigates the K5 graph
 *         on a 6.5s repeat) — the slide stays alive while the
 *         speaker takes questions; ambient breath continues forever
 *
 * Reduced-motion fallback: the constellation is drawn fully present
 * (no entrance), the title is fully visible, the underline is solid,
 * and the central pulse + perimeter wave + breath are all suppressed.
 * The composition still reads as deliberate — just without the build.
 */

// Five-pointed pentagon, apex at top, in a 600x600 viewBox. Each
// node binds to one QP2 theme — same vocabulary the audience saw
// previewed on slide 30 and exercised across 31 / 33. The node
// color band (amber/cyan/sage/violet/coral) carries the full deck
// palette so the finale resolves the deck's chromatic identity in
// the same gesture as its structural identity. Mapping is a fixed
// permutation: theme glyphs travel with their themes.js token, but
// the node-color decoration is a separate visual contract that
// makes the finale visibly chromatic.
const NODE_TOKENS = ['amber', 'cyan', 'sage', 'violet', 'coral'];
const NODE_THEMES = QP2_THEMES; // canonical 5; index aligns with NODE_TOKENS
const VIEWBOX = 600;
...
export default function Slide35ThankYou() {
  ...
  return (
    ...
    >
      ...
      <AuthorBlock T={T} ease={ease} prefersReduced={!!prefersReduced} />

      <ContactBlock T={T} ease={ease} prefersReduced={!!prefersReduced} />
    </motion.section>
  );
}
      ...
      >
        ...
        >
          ...
          {/* Nodes — five themed discs settling in from the deck's
              palette. Halo + inner disc + theme glyph blooming on top
              after the title resolves. The glyphs are the deck's
              consistent vocabulary (themes.js: ⇌ ◎ ⊕ ◇ ◈) so the
              audience reads "every theme is here, accounted for." */}
          <g filter="url(#finale-soft-glow)">
            {NODE_TOKENS.map((token, i) => {
              const v = VERTICES[i];
              const color = `var(--${token})`;
              const theme = NODE_THEMES[i];
              const delay = T.nodesStart + i * T.nodesStagger;
              const glyphDelay = T.glyphsStart + i * T.glyphsStagger;
              if (prefersReduced) {
                return (
                  <g key={token}>
                    <circle cx={v.x} cy={v.y} r={32} fill={color} fillOpacity={0.12} />
                    <circle cx={v.x} cy={v.y} r={14} fill={color} fillOpacity={0.85} />
                    <text
                      x={v.x}
                      y={v.y}
                      textAnchor="middle"
              ...
              return (
                <g key={token}>
                  ...
                  <motion.circle
                    ...
                    style={{ transformBox: 'fill-box', transformOrigin: 'center' }}
                  />
                  {/* Theme glyph — blooms onto the disc after the title
                      lands. Color is --bg (the canvas tone) so it reads
                      as a knockout against the colored disc. Slight
                      scale overshoot ties it to the node's settle. */}
                  <motion.text
                    x={v.x}
                    y={v.y}
                    textAnchor="middle"
                    ...
                    initial={{ opacity: 0, scale: 0.4 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.55, ease: overshoot, delay: glyphDelay }}
                    style={{ transformBox: 'fill-box', transformOrigin: `${v.x}px ${v.y}px` }}
                  >
                    {theme.glyph}
                  </motion.text>
          ...
          {/* Theme labels — small mono caption per node, placed
              radially outward so the title typography in the center
              never collides with them. They fade in last (after the
              speaker's "Thank you." has landed) so the constellation
              reveals its vocabulary as a quiet annotation, not as
          ...
          <g>
            {NODE_TOKENS.map((token, i) => {
              ...
              const theme = NODE_THEMES[i];
              const angle = -Math.PI / 2 + (i * 2 * Math.PI) / 5;
              // Push label 56 units outward along the same radial.
              const lx = CENTER + (RADIUS + 56) * Math.cos(angle);
              const ly = CENTER + (RADIUS + 56) * Math.sin(angle);
              const anchor =
                Math.cos(angle) > 0.3 ? 'start' :
                Math.cos(angle) < -0.3 ? 'end' : 'middle';
              const themeColor = `var(--${theme.token})`;
              return (
                <motion.text
                  key={`label-${theme.key}`}
                  x={lx}
                  ...
                  transition={{ duration: 0.5, ease, delay: T.themeLabels + i * 0.08 }}
...
function CenterTypography({ T, ease, prefersReduced }) {
  return (
    ...
    >
      ...
      {/* Calligraphic underline — replaces the prior straight bar.
          A subtle SVG bezier draws L→R via pathLength so the
          gesture reads like a single brushstroke under "Thank
          you." In reduced-motion the path is solid from t=0 with
          opacity 1; otherwise pathLength animates from 0 → 1 over
          0.8s with a soft cubic ease, plus a brief tail-thickening
          to mimic ink lifting off the page. */}
      <div
        style={{
          ...
          {prefersReduced ? (
            ...
            <>
              <motion.path
                ...
                transition={{
                  pathLength: { duration: 0.85, ease: [0.2, 0.7, 0.3, 1], delay: T.rule },
                  opacity:    { duration: 0.30, ease, delay: T.rule },
                }}
              />
              {/* Inkstroke shadow — fainter trail underneath the
                  brushstroke for a hint of weight; same draw timing,
                  slightly slower to lag behind the main stroke. */}
              <motion.path
                d="M 6 11 C 60 6, 140 16, 194 11"
                stroke="var(--amber)"
                strokeOpacity="0.35"
                strokeWidth="3.5"
                fill="none"
                ...
                transition={{
                  pathLength: { duration: 0.95, ease: [0.2, 0.7, 0.3, 1], delay: T.rule + 0.05 },
                  opacity:    { duration: 0.40, ease, delay: T.rule + 0.05 },
                }}
              />
            </>
          )}
...
/* ============================================================
   ...
   ============================================================ */
function AuthorBlock({ T, ease, prefersReduced }) {
  return (
    <motion.div
      className="absolute"
      style={{
      ...
      initial={prefersReduced ? false : { opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease, delay: prefersReduced ? 0 : T.author }}
    >
      <div
        className="deck-display"
        style={{
...
/* ============================================================
   ...
   ============================================================ */
function ContactBlock({ T, ease, prefersReduced }) {
  return (
    <motion.div
      className="absolute"
      style={{
      ...
      initial={prefersReduced ? false : { opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease, delay: prefersReduced ? 0 : T.contact }}
    >
