...
export default function RseStabilityCurve({
  ...
  // Point map for marker plotting.
  const ns = [20, 30, 40, 50, 60, 70, 80, 90, 100];
  const yForN = (n) => {
    // Decay: at N=20 → top (RSE high · y near y0+30), N=100 → bottom (low · y near y1-20)
    // Use exponential with knee around 55.
    const t = (n - 20) / 80; // 0..1
    const decay = Math.exp(-3 * t); // 1 at 0, ~0.05 at 1
    return y1 - 30 - decay * 0; // baseline
  };
  const points = ns.map((n) => {
    const x = x0 + ((n - 20) / 80) * (x1 - x0);
    const t = (n - 20) / 80;
    const decay = Math.exp(-3.4 * t);
    const y = y0 + 24 + decay * (y1 - y0 - 56);
    return { n, x, y };
  });
  void yForN;
