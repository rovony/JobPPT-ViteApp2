const cs3Notes: Record<string, string> = {
  'cs3-ivosidenib-divider': `## Spoken
Case three is ==ivosidenib== — approved in more than forty countries, but not in India.

The drug worked globally. The constraint was ==local evidence==.

Could a clinical pharmacology dossier do the job a local trial normally does?

## Cues
- ⏱ 25 sec — crisp reset; regulatory strategy case
- 🎚 Forty-plus approvals vs no India access
- ✅ Land on the dossier question

## Bridge
→ The SEC challenge — what they asked for in December 2024.`,

  'cs3-bg-disease': `## Spoken
By August 2024, ivosidenib had achieved marketing authorization in forty-two countries. This included every major reference regulator globally.

But **==India was not one of them==**.

With roughly 1.4 billion people, India represents a massive patient population. The lack of access was **==not because the science was immature==**. The drug had years of post-approval safety experience and a broad global dossier.

The roadblock was regulatory structure. India generally expected pre-approval local clinical data — usually a Phase 3 efficacy study or a bridging PK/PD trial — before registering a foreign-approved drug. For an orphan oncology drug, generating that local data could mean an 18 to 36 month delay.

So this slide poses the question for the case: can a global clinical pharmacology package register a drug in India **==without a local trial==**?

## Cues
- ⏱ 70 sec — Setup the conflict. 42 countries vs 1.4 billion people.
- 📍 Point to the India card when you say "India was not one of them."
- 🎚 Say "18 to 36 month delay" with gravity.
- ✅ Keep the thesis question clean: "without a local trial?"

## Bridge
→ Before we answer the regulatory question, let me quickly orient you to the biology and epidemiology of IDH1.`,

  'cs3-disease': `## Spoken
Let's do a quick biology orientation to ground the case.

IDH1 mutations are rare. They occur in about **==six to ten percent of AML patients==**, and about **==thirteen percent of intrahepatic cholangiocarcinoma patients==**. Both populations sit inside broader cancers, making trial recruitment challenging globally, let alone locally.

The mechanism is elegant. Mutant IDH1 produces an oncometabolite called 2-hydroxyglutarate (2-HG). This 2-HG disrupts epigenetic regulation and blocks the cell from differentiating. 

**==Ivosidenib inhibits that mutant IDH1 enzyme directly==**. It lowers 2-HG levels and relieves the block, allowing the cells to differentiate normally. This changed the landscape for relapsed or refractory AML starting with its first FDA label in July 2018.

One critical detail for the regulatory argument: this is a **==somatic tumor mutation==**, not an inherited germline variant. We'll come back to why that matters for ethnic sensitivity.

## Cues
- ⏱ 70 sec — Orient the panel. Do not get bogged down in the pathways.
- ⚠ Ensure you say "intrahepatic cholangiocarcinoma" clearly for the 13% figure.
- 🎚 Give extra weight to the last point: somatic vs germline. It's the lynchpin of the strategy.
- ✅ Land on: "somatic, not germline."

## Bridge
→ Let me show you the IDH inhibitor landscape to explain why ivosidenib was the focus.`,

  'cs3-competitors': `## Spoken
To understand the landscape, let's look at the timeline.

Mutations in IDH1 and IDH2 were first identified back in 2008 and 2009. By 2010, the field recognized 2-HG as the driving oncometabolite. This provided a very clear hypothesis: inhibit mutant IDH, suppress 2-HG, and restore cellular differentiation.

The first drug in the class to reach approval was enasidenib in 2017, but that targeted IDH2. **==Ivosidenib followed in 2018 as the first-in-class IDH1 inhibitor==**, initially for AML, and later expanded into cholangiocarcinoma.

Other molecules like olutasidenib and vorasidenib entered the field later, targeting different indications like glioma. But they are **==not part of this India registration story==**.

The focus here is specific: ivosidenib, targeting IDH1, for AML and cholangiocarcinoma, with global approvals already secured.

## Cues
- ⏱ 70 sec — Keep it moving. This is context, not a deep dive into competitors.
- ⚠ Emphasize that ivosidenib was the *first IDH1* inhibitor (not the first IDH inhibitor overall).
- ✅ Land on: "globally approved before India."

## Bridge
→ So how did the regulatory pathway in India suddenly change?`,

  'cs3-bg-regulatory': `## Spoken
India's 2019 rules expected local Phase III data. For rare IDH1-mutant AML, that means years of delay.

On ==August 7, 2024==, DCGI's Rule 101 order named six reference agencies and waiver categories — including orphan drugs and significant therapeutic advancement. Ivosidenib fit both.

Rule 101 does not guarantee approval. It opens the door. The scientific answer still has to come from the ==clinical pharmacology dossier==.

## Cues
- ⏱ 55 sec — regulatory pivot; speak precisely
- ⚠ Emphasize Rule 101 is opportunity, not automatic approval
- ✅ Land on clinical pharmacology dossier

## Bridge
→ Six convergent pillars in that dossier.`,

  'cs3-setup': `## Spoken
- Chronology first: ==Rule 101== opened the waiver path in **August 2024** — before the December SEC meeting you see on this slide.
- In December 2024 the SEC asked for a ==PK/PD study in India== before approval.
- Under the old framing, that request is reasonable. Under our evidence, it is a twelve-to-eighteen-month delay patients did not need.
- We already had PopPK on ==253 patients== with race not significant, PBPK-supported DDI labeling, and flat exposure-response.
- The strategy: prove the ==existing global clinical pharmacology package== answers the local-data question.

## Cues
- ⏱ 55 sec — thesis slide; steady conviction
- 🎚 Contrast SEC request vs existing PopPK evidence
- ✅ Land on "existing global package"

## Bridge
→ Rule 101 and the regulatory opening that made the waiver possible.`,

  'cs3-architecture-v2': `## Spoken
Let's build the argument, starting with mechanism.

First, **==the mutation is somatic==**. It is acquired only in the tumor cells. If you look at the cell strip on the left, only the middle cell is mutant; the neighboring cells are healthy. If this were a germline mutation, every cell would carry it, and patient ancestry would be a major confounder. It is not.

Second, ivosidenib binds directly to the mutant IDH1 enzyme. The cascade is direct: mutant IDH1 drives 2-HG production, which inhibits dioxygenases and blocks differentiation.

Third, when ivosidenib inhibits that mutant enzyme, **==the entire cascade reverses==**. 2-HG levels drop, the epigenetic block is lifted, and differentiation resumes.

Why does this matter to the Indian regulator? Because the drug target exists **==exclusively in the tumor, not inherited across the patient's ethnic lineage==**. This allows us to argue ethnic insensitivity based on **==biology, not just statistics alone==**.

## Cues
- ⏱ 85 sec — Walk through the three beats systematically: somatic, binds, reverses.
- 📍 Point to the three-cell strip to emphasize the somatic nature of the mutation.
- 🎚 The climax of the slide is the link between the somatic biology and the ethnic insensitivity argument.
- ✅ Land on: "from biology, not just statistics alone."

## Bridge
→ That mechanism was just the first of six convergent pillars in the dossier.`,

  'cs3-pillars': `## Spoken
The waiver case rested on ==six converging lines of evidence==.

Mechanism — somatic IDH1, not germline. PK similarity — two hundred fifty-three patients, race not significant. Exposure-response — flat across range, five hundred milligrams once daily.

Intrinsic and extrinsic factors fully characterized. Global regulatory experience across thirty-plus jurisdictions.

==No single pillar wins alone. Convergence is the case.==

## Cues
- ⏱ 70 sec — name six pillars distinctly; keep pace
- 🎚 Count pillars if natural; don't rush the convergence line
- ✅ Land on "Convergence is the case"

## Bridge
→ Did the strategy work? The public timeline.

## Off-slide (if asked — not on screen)
- **Pillar detail** (compressed on slide): (1) somatic IDH1 mechanism · (2) PopPK N=253, race NS · (3) flat E-R, 500 mg QD · (4) intrinsic/extrinsic factors · (5) 30+ global jurisdictions · (6) Phase 4 PK/PD commitment named upfront
- **36-page justification** document — mechanism-first, not statistics-only ethnic bridging
- Backup: \`cs3-B2-six-pillar-package\``,

  'cs3-reversal': `## Spoken
The public arc: FDA approvals from 2018 through 2021, EMA in 2023, then ==August 2024== when Rule 101 opened the waiver pathway.

The outcome: ==May 14, 2025== — CDSCO marketing authorization in India without a pre-approval local trial.

The authorization followed accumulated global evidence and a pharmacology package that made extrapolation ==scientifically defensible==.

## Cues
- ⏱ 55 sec — walk timeline left to right
- 🎚 State May 14, 2025 as fact, not boast
- ✅ Land on scientifically defensible extrapolation

## Bridge
→ An honest accounting of what we shipped and what we did not.`,

  'cs3-reckoning': `## Spoken
Clinical pharmacology requires an ==honest accounting==.

We shipped a mechanism-first defense, a thirty-six-page justification on six pillars, and CDSCO approval with a Phase 4 commitment.

We did not have pre-approval Indian PK/PD data, Indian pivotal sites, or an India-specific peer-reviewed PK paper. Inference rested on the two hundred fifty-three-patient PopPK model.

The dossier was ==explicit about what remained uncertain== — and the Phase 4 commitment addressed it.

## Cues
- ⏱ 55 sec — equal weight to shipped vs gaps
- 🎚 Candor builds credibility with the panel
- ✅ Land on explicit uncertainty + Phase 4

## Bridge
→ How the team organized to execute under pressure.

## Off-slide (if asked — not on screen)
- **Amber "honest accounting" band removed** — still deliver candor verbally
- **Shipped:** mechanism-first defense · six-pillar package · CDSCO approval · Phase 4 commitment
- **Not shipped pre-approval:** Indian PK/PD · Indian pivotal sites · India-specific peer-reviewed PK paper
- **Residual uncertainty:** explicit in dossier; Phase 4 closes the PK gap post-approval`,

  'cs3-leadership': `## Spoken
The operating model had four clear owners.

==Quantitative pharmacology== translated ICH E5 concepts into PopPK, exposure-response, and PBPK evidence — and defined the Phase 4 PK/PD commitment.

==Regulatory affairs== owned the Rule 101 pathway and agency responses. Medical and pharmacovigilance owned post-marketing follow-up. The India affiliate owned in-country execution.

Quantitative pharmacology answered: can global evidence extrapolate? Partner functions answered: can the pathway hold?

==Both had to be true== for CDSCO to grant the waiver.

## Cues
- ⏱ 55 sec — function-level language; cross-functional tone
- ✅ Land on "Both had to be true"

## Bridge
→ Three portable lessons from this case.

## Off-slide (if asked — not on screen)
- **Amber thesis removed from slide:** "Quantitative pharmacology answered whether global evidence extrapolated; partner functions answered whether the pathway could hold — both had to be true."
- **Phase 4 owner:** quantitative pharmacology defined PK/PD commitment scope
- **India affiliate:** in-country execution under SEC/CDSCO cadence
- **Leadership signal:** translating science into a cross-functional operating model under deadline`,

  'cs3-bridge-recap': `## Spoken
==The science was the bridge.==

Three lessons: mechanism matters — somatic IDH1 narrowed the ethnic-sensitivity question. Convergence matters — no single pillar carried the waiver alone. Transparency matters — we named the PK gap and committed to Phase 4.

When a local trial is not feasible, a rigorous clinical pharmacology dossier can become the ==bridge to patient access==.

## Cues
- ⏱ 40 sec — synthesis; count three lessons cleanly
- ✅ Land on bridge to patient access

## Bridge
→ Case four shifts medium — AI infrastructure — same standard: traceable decisions.`,
};

export default cs3Notes;
