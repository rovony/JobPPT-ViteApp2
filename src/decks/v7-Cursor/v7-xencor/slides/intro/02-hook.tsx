 ...
 *
 * Reveal: marks fade in progressively (~0.55s + 0.1s stagger).
 * Solid panel + top case-color accent (light-editorial paper).
 *
...
/* Presentation order: Ambrisentan → India → Asparlas → Pharazi */
const MARKS = [
...
export default function HookATrialNotAnswer({ deck }) {
  ...
  // Never gate visibility on isInView && !reduced — that leaves opacity 0 stuck.
  const go = isInView || prefersReduced;
  const d = (ms) => (prefersReduced ? 0 : Math.min(ms, 0.35));

  const fade = (delay) => ({
    initial: prefersReduced ? false : { opacity: 0 },
    animate: { opacity: 1 },
    transition: { duration: prefersReduced ? 0 : 0.35, delay: d(delay), ease: [0.2, 0.7, 0.3, 1] },
  });
  ...
  return (
    <TitleLayout deck={deck}>
      ...
      >
        {/* ─── Xencor wordmark — shared layoutId from slide 01.
        ...
        >
          ...
          >
            {/* Hook headline echoes deck title verbatim — the premise IS
                the abstract thesis: when a conventional clinical trial
                cannot provide an answer, clinical pharmacology closes
                the gap. The italic-amber payoff on "can't answer."
                creates visual rhyme with the slide-1 title's italic-
                amber pivot on "answer." — same word, same color, same
                weight. The cases (slides 5+) ground the abstract in
                concrete reality, NOT this hook. */}
            ...
            >
              <em style={{ color: 'var(--amber)', fontStyle: 'italic', fontWeight: 500 }}>
                falls short.
              </em>
            </span>
          </h1>
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    borderTop: `3px solid ${m.foreshadow}`,
