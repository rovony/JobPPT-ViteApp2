...
export const TYPE = {
  hero: 'clamp(2.5rem, min(6vw, 9vh), 4.5rem)',
  heroSm: 'clamp(2rem, min(4.5vw, 7vh), 3.5rem)',
  title: 'clamp(1.05rem, min(1.6vw, 2.4vh), 1.35rem)',
  body: 'clamp(0.9rem, min(1.25vw, 2vh), 1.15rem)',
  label: 'clamp(0.7rem, min(0.95vw, 1.5vh), 0.95rem)',
};
...
export const INK = {
  ...
  panel: 'color-mix(in srgb, var(--panel) 72%, transparent)',
};

export const SPACE = {
  gap: 'clamp(0.75rem, 1.5vw, 1.5rem)',
  pad: 'clamp(1rem, 2vw, 1.5rem)',
  padLg: 'clamp(1.15rem, 2.4vw, 1.75rem)',
};
...
export function AmbientLungs({ variant = 'context', opacity = 0.05 }) {
  return (
    <div
      aria-hidden
      style={{
    ...
    >
      <Lungs layoutId="cs1-lung" variant={variant} widthOverride="100%" />
    </div>
  );
}
