...
export const overrideNotes: Record<string, string> = {
  title: note(
    `Good morning, and thank you for the time. I am Malek Okour. Over the next forty-five minutes I want to walk you through four clinical pharmacology decisions ==where the clean measurement was never going to be enough==.
...
One discipline sits at the center of all four: **==when measurement falls short, clinical pharmacology makes the dose defensible==**.`,
...
- Meta line: Xencor candidate seminar · ==June 17, 2026== · ~45 min including Q&A`
  ),

  'hook-A-trial-not-answer': note(
    `**==When measurement falls short==** — that is the premise behind every case in this talk.

The constraint shows up in four ways, and you will see each one on the roadmap.

==Untrialable== means you cannot run the efficacy trial because ethics and disease biology forbid it. ==Sample-limited== means the endpoint-powered design is clean on paper and undeliverable in practice. ==Local evidence== means a regulator asks for data the global program never enrolled. ==Unbuilt== means the next decade needs audit-ready infrastructure that no vendor ships off the shelf.
...
After the cases I will widen the aperture to portfolio breadth, then close with a Xencor-specific bridge. This is not a methods catalog — it is a sequence of decision constraints and how clinical pharmacology answered each one.`,
    `- ⏱ 75 sec — point to each case card as you name it
- ⚠ Portfolio and Xencor bridge come after the four cases
