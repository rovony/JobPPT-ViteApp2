# Overview · Pre-talk briefing

> **Read time:** ~5 min · **For:** the presenter, before stepping on stage

This briefing is for the Xencor candidate seminar on June 17, 2026.
It is meant to keep the live story grounded while leaving detailed source
defenses in backup slides.

## What this deck argues

When measurement falls short, clinical pharmacology makes the dose defensible. The talk
uses four cases to show the same leadership move in different evidence
constraints: pediatric extrapolation, efficient design, India reliance, and
audit-ready clinical pharmacology workflows.

## Four cases at a glance

| Case | Therapeutic area | Key analytic move | Regulatory outcome |
|---|---|---|---|
| CS1 | Pediatric PAH | Model-informed dose for an untriable trial | EMA + PMDA approval |
| CS2 | Asparlas / adult ALL | Simulated primary plus optimal design | FDA Type A agreement on N=60 |
| CS3 | Ivosidenib / India | Six-pillar reliance dossier | CDSCO approval without local pre-approval trial |
| CS4 | AI / Pharazi | Traceable workflow architecture | Personal research platform |

## Things to remember before the room

- Keep the main flow to ~45 minutes including Q&A.
- The audience is Xencor clinical pharmacology / translational PK/PD leadership.
- ==Avoid Servier internal numbers== throughout.
- Do not claim direct Xencor or PRO-XTEN program ownership.
- Keep ADC framed only as portfolio breadth if asked, not as a core case.
- **Slides were de-busied on purpose.** Trimmed copy lives in **speaker notes → Off-slide** sections, per-slide **Q&A**, and **Reading → Off-slide memory · cheat sheet**.

## Open questions to revisit

- [ ] Confirm final PDF export and presenter route.
- [ ] Decide whether to rehearse with backups visible in the slide rail.
- [ ] Final pass on timing after browser proof.
