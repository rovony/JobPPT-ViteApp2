    ...
    <SlideGrid dataCase="amber" areas={STANDARD_AREAS}>
      ...
      <Viz>
        <div ref={ref} style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          {/* Main Timeline Area — overflow:hidden + minHeight:0 so the
              ascending IV-bag stop on the far right cannot push past the
              viz cell on non-XL viewports. */}
          <div style={{ flex: 1, position: 'relative', marginTop: 'var(--space-2)', minHeight: 0, overflow: 'hidden' }}>
            ...
            {/* Cards along an ascending curve.
                minmax(0, 1fr) prevents grid-text blowout (the long stop
                titles in narrow columns were wrapping past the cell). */}
            <div style={{
              display: 'grid',
              ...
              {STOPS.map((stop, i) => {
                /* Ascending curve — vh-based with px ceiling so the rise
                 * doesn't push icons off-canvas on narrower / shorter
                 * viewports. Reduced ~40% from prior values (4/9/17/26vh
                 * → 2.5/6/11/16vh) so the IV-bag stop on the far right
                 * stays inside the viz cell at non-XL viewports. The
                 * amber spine SVG still rises visibly across the row. */
                const curve = ['0px', 'clamp(0px, 2.5vh, 18px)', 'clamp(0px, 6vh, 44px)', 'clamp(0px, 11vh, 88px)', 'clamp(0px, 16vh, 132px)'];
                const mb = reduced ? '0px' : curve[i];
                ...
                >
                  {/* Icon — tighter scale at narrow viewports so the IV bag
                      doesn't punch into the headline area on ≤1366px screens */}
                  <div style={{
