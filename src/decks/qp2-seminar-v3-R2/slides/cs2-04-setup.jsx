...
const IN_HAND_ITEMS = [
  ...
  { text: 'PBPK-supported DDI label · midazolam AUC ratio 0.18', cite: '— Xu et al., CPT:PSP 2021', pmcUrl: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC8213421/' },
...
export default function CS2Setup() {
  return (
    <SlideFrame
      ...
      footerSource="CDSCO SEC public minutes · 10 Dec 2024"
    >
      <div
        style={{
          position: 'absolute',
    ...
    <div style={{ flexShrink: 0 }}>
      ...
      >
        ...
        <motion.line
          ...
          stroke="#5dc8de" strokeWidth="1" strokeOpacity="0.35"
          initial={reduced ? false : { pathLength: 0 }}
          animate={{ pathLength: 1 }}
        ...
        <TimelineTick cx={80} date="10 MAR 2022" label="EMA filing"
          delay={D.ticks123} reduced={reduced} />
        <TimelineTick cx={380} date="10 MAY 2023" label="EMA approval · AML+CCA"
          delay={D.ticks123} reduced={reduced} />
        <TimelineTick cx={720} date="27 MAR 2024" label="CDSCO MAA filed"
          delay={D.ticks123} reduced={reduced} />

        {/* Tick 4 — SEC opinion with overshoot + ambient pulsing ring */}
        <motion.circle
          cx="1100" cy="44" r="7" fill="#5dc8de"
        ...
        <motion.text
          ...
          fontSize="11" fill="#5dc8de" letterSpacing="1.4" fontWeight="600"
        ...
        <motion.text
          ...
          fontSize="12" fill="#5dc8de" fontStyle="italic" fontWeight="500"
          initial={reduced ? false : { opacity: 0 }}
          animate={{ opacity: 1 }}
...
function TimelineTick({ cx, date, label, delay, reduced }) {
  return (
    ...
    >
      <circle cx={cx} cy="44" r="5" fill="#0f1011" stroke="#6e6a5e" strokeWidth="1.5" />
      <text x={cx} y="22" textAnchor="middle" fontFamily="'JetBrains Mono', monospace"
        fontSize="10" fill="#6e6a5e" letterSpacing="1.4">{date}</text>
      <text x={cx} y="66" textAnchor="middle" fontFamily="'Fraunces', Georgia, serif"
        fontSize="12" fill="#8a8479" fontStyle="italic">{label}</text>
    </motion.g>
  );
}
      ...
      >
        ...
        >
          <div
            ...
            style={{
              fontSize: 'clamp(0.68rem, min(1vw, 1.4vh), 0.88rem)',
          ...
          <div
            ...
            style={{
              ...
              fontSize: 'clamp(0.6rem, min(0.85vw, 1.2vh), 0.78rem)',
              letterSpacing: '0.14em',
              color: 'var(--cream-muted)',
            }}
          >
            10 December 2024 · Meeting Minutes (Public Record)
          </div>
        </motion.div>
        ...
        <div
          ...
          style={{
            margin: 0,
            fontSize: 'clamp(2rem, min(3.8vw, 5.5vh), 3.2rem)',
            lineHeight: 1.15,
        ...
        >
          <span
            className="deck-mono uppercase"
            style={{
              fontSize: 'clamp(0.58rem, min(0.8vw, 1.1vh), 0.72rem)',
              letterSpacing: '0.14em',
              color: 'var(--cream-muted)',
            }}
          >
...
function AsymmetryPanel() {
  ...
  return (
    ...
    >
      {/* LEFT — Asked (muted, cascades first) */}
      <div>
        <motion.div
          className="deck-mono uppercase"
          initial={reduced ? false : { opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.2, ease: EASE, delay: D.asked }}
          style={{
            fontSize: 'clamp(0.68rem, min(1vw, 1.4vh), 0.88rem)',
            letterSpacing: 'var(--ls-mono-wide)',
            color: 'var(--cream-muted)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(5px, 0.7vh, 10px)' }}>
          {ASKED_ITEMS.map((item, i) => (
            <motion.div
              key={item}
              initial={reduced ? false : { opacity: 0, x: -8 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.2, ease: EASE, delay: D.asked + 0.1 + i * D.askedStagger }}
              style={{ display: 'flex', alignItems: 'baseline', gap: 'clamp(8px, 1vw, 14px)' }}
            >
              <span className="deck-mono" style={{ fontSize: '0.8rem', color: 'var(--cream-faint)' }}>·</span>
              <span
                style={{
                  fontFamily: 'var(--font-body)',
                  fontSize: 'clamp(0.82rem, min(1.2vw, 1.8vh), 1.08rem)',
                  color: 'var(--cream-muted)',
                  lineHeight: 1.45,
                }}
              >
      ...
      {/* RIGHT — Already in hand (cyan, cascades after ASKED) */}
      <div>
        <motion.div
          className="deck-mono uppercase"
          initial={reduced ? false : { opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.2, ease: EASE, delay: D.inHand }}
          style={{
            fontSize: 'clamp(0.68rem, min(1vw, 1.4vh), 0.88rem)',
            letterSpacing: 'var(--ls-mono-wide)',
            color: 'var(--cyan)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(5px, 0.7vh, 10px)' }}>
          {IN_HAND_ITEMS.map((item, i) => (
            <motion.div
              ...
              initial={reduced ? false : { opacity: 0, x: -8 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.2, ease: EASE, delay: D.inHand + 0.1 + i * D.inHandStagger }}
              style={{ display: 'flex', alignItems: 'baseline', gap: 'clamp(8px, 1vw, 14px)' }}
            >
              <span className="deck-mono" style={{ fontSize: '0.8rem', color: 'var(--cyan)' }}>·</span>
              <span
                style={{
                  fontFamily: 'var(--font-body)',
                  fontSize: 'clamp(0.82rem, min(1.2vw, 1.8vh), 1.08rem)',
...
function CitationLink({ cite, href }) {
  ...
  return (
    <span
      ...
      style={{
        fontSize: 'clamp(0.65rem, min(0.9vw, 1.3vh), 0.82rem)',
        fontStyle: 'italic',
        color: 'var(--cream-muted)',
    ...
    >
      <p
        ...
        style={{
          margin: 0,
          fontStyle: 'italic',
          fontSize: 'clamp(0.88rem, min(1.4vw, 2.2vh), 1.25rem)',
          lineHeight: 1.4,
          color: 'var(--cream)',
        }}
      >
        <span style={{ color: 'var(--amber)', fontStyle: 'normal', fontWeight: 600, marginRight: 10 }}>
          ▌
        </span>
        The decision:{' '}
