...
function clearAutoDismiss(id) {
  ...
  if (t) { clearTimeout(t); autoDismissTimers.delete(id); }
}
...
function toast({ duration, ...props }: any) {
  ...
  const update = ({ duration: newDur, ...rest }: any) => {
    dispatch({ type: actionTypes.UPDATE_TOAST, toast: { ...rest, id } });
    if (typeof newDur === 'number') scheduleAutoDismiss(id, newDur);
  };
