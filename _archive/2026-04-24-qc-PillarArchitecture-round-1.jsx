  ...
  return (
    <motion.div
      ...
      style={{
        ...
        gap: isHero ? 'var(--space-3)' : isSeed ? 2 : 6,
        minHeight: 0,
        position: 'relative',
        overflow: 'hidden',
        justifyContent: isHero ? 'flex-start' : isSeed ? 'center' : 'flex-start',
      }}
    >
      ...
      {/* Tag (one-line eyebrow under the name) */}
      {(isFull || isChain) && (
        <div
      ...
      {/* Outcome (the one-sentence verdict per pillar) — only on full */}
      {isFull && (
        <div
          style={{
