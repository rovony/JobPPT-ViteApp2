 ...
 * the display tier (3–6rem), one tier above --fs-card-hero-num
...
// Three station x positions inside the 0–1000 armature viewBox, at
// 1/6, 1/2, 5/6 — matches the centers of the three CSS columns when
// the column gap is small relative to the column width.
const STATION_X = [167, 500, 833];
...
/* ========================================================
   ...
   ======================================================== */
function TakeColumn({ t, colDelays, reduce, ease }) {
  ...
  return (
    ...
    >
      {/* Row 1 — dot/eyebrow, sized to ARM_TOP% so its center sits on
          the SVG hairline. The bg color clip keeps the hairline from
          ghosting through the eyebrow type. */}
      <motion.div
        style={{
          alignSelf: 'center',
          display: 'flex',
      ...
      {/* Row 2 — hero numeral at the display tier. Sits below the
          eyebrow with a comfortable lead. */}
      <div
        style={{
          display: 'flex',
          alignItems: 'baseline',
      ...
      >
        <Metric t={t} delay={colDelays.numeral} reduce={reduce} ease={ease} />
      </div>
...
/* ========================================================
   ...
   ======================================================== */
function Metric({ t, delay, reduce, ease }) {
  const numStyle = {
    fontSize: 'clamp(3rem, min(5.4vw, 8.4vh), 6rem)',
    fontWeight: 700,
    lineHeight: 1,
