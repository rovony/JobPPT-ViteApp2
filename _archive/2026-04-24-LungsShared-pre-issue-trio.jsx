...
export default function LungsShared({
  ...
  return (
    <motion.div
      layoutId={layoutId}
      layout
      // Context (slide 6) lung mounts at opacity:1, NOT framer-motion's
      // default 0->1 layoutId crossfade ramp. The crossfade was causing
      // the lung to appear at ~10% opacity during the first ~200ms of
      // the 5->6 morph, while the source lung in slide 5 was simultaneously
      // dimming with SlideTransition's exit (0.4s) — net effect: lung
      // looked "ghosted" for ~half a second mid-morph. Locking the target
      // to opacity:1 means the morphing element is always fully painted;
      // only the source-side fade behind SlideTransition still happens,
      // and that just looks like the rest of the slide leaving.
      initial={isHero ? { opacity: 0, scale: 0.92 } : { opacity: 1 }}
      animate={isHero ? { opacity: 1, scale: 1 } : { opacity: 1 }}
      transition={{
