...
export default function Slide11fCaseExposureResponse() {
  ...
  // Entrance choreography (V2 · 2026-04-24) — aligned with sibling slides
  // 10 (pcVPC) and 11e (exposure-match). The previous timeline ran panel A
  // and panel B in parallel (boxA: 1.00, boxB: 1.20) and let the closing
  // strip land at 2.85 — well before the second MedianGuide resolved. The
  // viewer's eye couldn't finish one panel before the next jumped in, and
  // the closing arrived while the chart was still settling. Net effect:
  // slide 12 read faster and "different" from its CS1 results siblings.
  //
  // New pace: panel A's medians + MedianGuide settle, THEN panel B's
  // medians + MedianGuide sequence in, THEN the closing caption + theme
  // pills land. Mirrors slide 11e's serial AUC → Cmax → closing rhythm.
  const D = {
    chrome: 0.10, headline: 0.25, subhead: 0.55,
    // Panel A (AUC, left) — hero beat is the two medians drawing L→R.
    // Box-stagger inside BoxGlyph is 0.15s → second median ends at 1.80.
    // MedianGuide on this panel resolves at boxA + 1.0 + 0.8 + 0.4 ≈ 3.05.
    axisA: 0.70, boxA: 0.85,
    // Panel B (Cmax, right) — sequenced AFTER Panel A's medians settle
    // (1.80) so the eye finishes one panel before the next begins. Mirrors
    // slide 11e's boxPeds: 1.95.
    axisB: 1.80, boxB: 1.95,
    // Closing strip lands AFTER Panel B's MedianGuide Δ pill resolves
    // (boxB + 1.0 [guide delay] + 0.8 [connector] + 0.4 [pill fade] ≈ 4.15).
    caption: 4.20, pills: 4.45, source: 4.65,
  };
      ...
      >
        ...
        <g transform={`translate(${m.left},${m.top})`}>
          ...
          {/* Median-overlap guide — dashed horizontal between the two medians.
              delay = boxDelay + 1.0 fires the connector right as the second
              median finishes drawing (boxDelay + 0.15 + 0.8 = boxDelay + 0.95). */}
          <MedianGuide
            tk={tk}
            xs={xs}
            y={y}
          ...
          {/* Tooltip — render last so it draws on top of boxes / guide. */}
          {hover !== null && (() => {
  ...
  return (
    <g style={{ pointerEvents: 'none' }}>
      ...
      {/* Median — in-chart emphasis. Draws L→R via pathLength, sequenced
          ...
          Rendered as <motion.path> not <motion.line> because framer-motion
          normalizes pathLength via getTotalLength(); SVG <line> reports a
          length of 0 in some browsers and the dash-offset trick collapses,
          drawing the stroke fully on the first frame (matches slide 11e's
          median path). Halo pulse removed 2026-04-24 — sibling slides
          have no equivalent flourish; pathLength draw carries the beat. */}
      <motion.path
      ...
      {/* Hover hit target — invisible, spans the full whisker height
          plus a small buffer so the cursor catches even at the caps.
          Sits above the visible glyph so it absorbs all mouse events. */}
      {onHover && (
        <rect
          x={cx - bw / 2 - 6}
          y={yMax - 8}
  ...
  return (
    <g>
      {/* Dashed connector between medians — draws L→R via pathLength
          over 0.8s (matches the bracket-draw pace on sibling slide 11e).
          Visual centerpiece: the connector is nearly horizontal = flat
          signal across the two groups = no exposure-response gradient.
          <motion.path> not <motion.line> for the same reason as the box
          median above (line.pathLength normalization fails in browsers). */}
      <motion.path
        ...
        transition={{ duration: reduce ? 0 : 0.8, ease, delay: reduce ? 0 : delay }}
      />
      {/* Δ pill — single fade-in after the connector lands.
          The previous opacity pulse [0, 1, 0.72, 1] was a flicker flourish
          with no analogue on slide 11e; removed 2026-04-24 to match family. */}
      <motion.g
