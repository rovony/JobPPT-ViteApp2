# DecisionGate v6.9.1 — pre-card-wrapper snapshot (archived 2026-04-24)

Snapshot of `src/decks/qp2-seminar/slides/cs1-build/DecisionGate.jsx` immediately
before two changes were applied on 2026-04-24:

1. **Card wrapper** — slide 09 (`Slide11cCaseBuild`) wraps the workflow column
   in a panel card matching the schematic panel on the left, instead of
   rendering DecisionGate flat against the slide background.

2. **Arrow alignment audit** — A1, A2, A3 endpoints adjusted so they land on
   the diamond's actual rotated-rect vertices (top 59.3, right 270.7,
   bottom 200.7), instead of overshooting / starting ~19px inside the
   diamond. The diamond description was repositioned cleanly below the
   diamond bottom (no more `var(--bg)` backdrop hack), and the rebuild
   node moved down to accommodate. Step 03 was nudged up 3px so the YES
   arrow lands exactly on its vertical center.

Verbatim copy of the file is below.
