...
| **D-tail — final morphs** | `ae771fe` + `397668e` | ✅ done | `cs1-focal-amber` layoutId pair (slide 11 ↔ 11b) + slide 22 SEC card wrapped in right-aligned `max-width:min(56%,640px)` strip so the `cs2-sec-objection` morph reads as hero→corner-badge collapse. |
| **D-density — slide 29 grid + slide 30 overlap** | `5924342` | ✅ done | Slide 29 declared 3 rows for 4 children; expanded to 4 explicit rows. Slide 30 type column switched from fixed-min `width: clamp(620px,...)` to `left+right` anchoring (eliminates ~200px overlap with constellation at <1280px). Slide 30 page-number tag changed `IV / IV` → `30 / 35` to remove duplicate of meta-footer Act marker. |
| **D-a11y — reduced-motion + P5 docs** | `6c51398` | ✅ done | Slide 32 `CountUpDigit` now snaps to target when `prefers-reduced-motion` is set (framer-motion `useReducedMotion()`). CLAUDE.md gains documented case-color contract (coral/cyan/violet → CS1/CS2/CS3; amber/sage non-case) + `--fs-card-*` token convention. |
| **E — confidentiality scrub** | (Apr 24 closing sweep) | ✅ done | 7 zaj-slides HARD-RULE violations remediated: slides 16/19/20/22 Servier→TIBSOVO USPI; slide 26 + notes.js FDA Type A verbatim → paraphrased regulatory positions (line numbers + decorative quote glyph removed); slide 31 "Internal Servier programs" → "author's pharmacometrics record". Public-source citations only. |
| **F — P5 case-color contract** | (Apr 24 closing sweep) | ✅ done | Slide 33 PRINCIPLES + RESEARCH_CARDS reassigned to amber/cream/sage (case-callback coral/violet preserved on bottom ribbon only); `themes.js` QP2_THEMES tokens remapped to alternate amber/sage so cross-case theme tiles never inherit case identity. |
| **G — `cs3-n-94` layoutId pair** | `f53924a` | ✅ done | Slide 24 AnchorTile (middle "94" tile) ↔ slide 26 SampleSizeWaterfall left bar. AnchorTile gains optional `numeralLayoutId` prop; slide 26 SVG `<text>` carries the matching `layoutId="cs3-n-94"`. Same SVG↔HTML partial-morph trade-off the existing `cs3-pct-36` pair accepts. |
| **H — repo hygiene** | (Apr 24 closing sweep) | ✅ done | `npm run clean:screenshots` script added; CLAUDE.md gains `src/lib/` vs `src/utils/` documentation (default to `lib/`; `utils/index.ts` is base44-scaffold leftover) + dead-code audit note for `src/components/slides/` (only `AnalysisPlot.tsx` + `analysis-plot-data.ts` remain in active use; rest was folded into deck-local `cs*-*` folders during structural sweep). |
| **D-defer — slide 21 rebuild** | _pending_ | 📋 deferred | Speaker-input-required item. See `DEFERRED-WORK.md` §1. |

**Apr 24 closing sweep landed Phases E / F / G / H on top of the original A → D-a11y batch.** Build green throughout (`vite build` exits 0). Each commit independently revertable. The single remaining deferred item (slide 21 proportional-time rebuild) genuinely requires speaker input — see `DEFERRED-WORK.md`.
